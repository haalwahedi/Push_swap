...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/22 15:01:52 by halwahed          #+#    #+#             */
/*   Updated: 2022/04/22 15:01:53 by halwahed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	case_with_10(t_stk **a, t_stk **b, int len, t_var *v)
{
	...
	while (i > 3)
	{
		min = get_min(*a);
		temp1 = (*a)->next;
		if ((*a)->num != min && temp1 && temp1->num == min)
			pick_case(a, b, SA, v);
		while (min != (*a)->num)
			normalize(a, b, v, min);
		if (min == (*a)->num)
...
void	case_with_3(t_stk **a, t_stk **b, int len, t_var *v)
{
	...
	temp = ft_lst_bottom(*a);
	min = get_min(*a);
	while (check_sort(a, len) == 0)
	{
		if ((*a)->num < (*a)->next->num && temp->num == min)
			pick_case(a, b, RRA, v);
		else if ((*a)->num > (*a)->next->num && temp->num == min)
			sort_3(a, b, v, RRA);
		else if ((*a)->next->num == min)
		{
			if ((*a)->num < temp->num)
				pick_case(a, b, SA, v);
			else if ((*a)->num > temp->num)
				pick_case(a, b, RA, v);
		}
		else if ((*a)->num == min && (*a)->next->num > temp->num)
			sort_3(a, b, v, RA);
	}
}
